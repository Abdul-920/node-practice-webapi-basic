...
exports.stubPreview =  catchAsync( async (req, res, next) => {

    const stubTemplate = await stubService.getStubTemplate(req.body);
    const pdfDoc = pdfMake.createPdf(stubTemplate);
    pdfDoc.getDataUrl((binary) => {
        res.send(binary);
    })
